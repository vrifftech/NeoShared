...
usage() {
  cat <<'USAGE'
usage: setup-wasm.sh --deps-root DIR [--force]

Installs the pinned Emscripten SDK and checks out the pinned PCBJam/wxWidgets
wasm-port commit under DIR. Nothing is installed globally.
USAGE
}
...
cat > "$DEPS_ROOT/neo-wasm-versions.env" <<EOF
...
EOF
printf 'Emscripten: %s\nwxWidgets-WASM: %s\n' "$EMSCRIPTEN_VERSION" "$WX_WASM_COMMIT"
