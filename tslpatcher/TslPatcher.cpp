...
IniMergeReport writeFragmentText(const std::string& fragmentText,
        ...
        outputIni.string() + ".neo-fragment-tmp-" + std::to_string(stamp);
    {
        ...
        output.write(fragmentText.data(), static_cast<std::streamsize>(fragmentText.size()));
        if (!output) {
            std::filesystem::remove(temporary);
            throw std::runtime_error(
    ...
    // Count only actual fragment sections and entries. This parser is purposely
    // narrow: fragmentText is generated by writeIniFragmentText and has no
    // comments or [Settings] section.
    std::istringstream input(fragmentText);
    std::string line;
...
IniMergeReport writeFragment(const PatchProject& project,
                             const std::filesystem::path& requestedIni) {
    return writeFragmentText(writeIniFragmentText(project), requestedIni);
}
